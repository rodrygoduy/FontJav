...
const Booking = () => {
    ...
    const handleSearch = (e) => {
        ...
        setSearchTerm(value)
        const filtered = bookings.filter((booking)=> booking.khachHang.hoTen.toLowerCase().includes(value))
    ...
    const handleDelete = (maDatPhong) =>{
        Modal.confirm({
            ...
            cancelText: "Hủy",
            onOk: () => {
                axios.delete(`home/booking/${maDatPhong}`)
                .then(()=>{
    ...
    const handleAddBooking = (values) => {
      const payload = {
        ...values,
        ngayNhanPhong: values.ngayNhanPhong, // Chuỗi 'YYYY-MM-DD'
        ngayTraPhong: values.ngayTraPhong, // Chuỗi 'YYYY-MM-DD'
      };
    ...
    return (
        ...
        <h1 style={{ fontSize: "24px", fontWeight: "bold", color: "#333" }}>Quản Lý Booking</h1>
    ...
    <div>
      ...
      <ul>
        {selectedBooking.suDungDichVuList.map((dichVu, index) => (
          <li key={index}>
            Dịch vụ: {dichVu.dichVu.tenDichVu}, Giá: {dichVu.dichVu.giaDichVu}, Số lượng: {dichVu.soLuong}
          </li>
        ))}
      </ul>
